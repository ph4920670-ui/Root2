...
def _run(sql, params=None):
    """Executa SQL com params posicionais [v1, v2, ...] em sintaxe :1, :2, ...

    pg8000.native aceita SÓ placeholders nomeados (`:nome`) com kwargs.
    Convertemos `:N` → `:pN` no SQL e passamos `{p1: v1, p2: v2, ...}`.
    """
    import re as _re2
    conn = _get_conn()
    if params:
        fixed = _re2.sub(r':(\d+)', lambda m: ':p' + m.group(1), sql)
        kwargs = {f'p{i+1}': v for i, v in enumerate(params)}
    else:
        fixed = sql
        kwargs = {}
    try:
        return conn.run(fixed, **kwargs) if kwargs else conn.run(fixed)
    except Exception:
